...
var SGS_categories = {
    ...
    constructNode: function (node, parentElement) {
        ...
        content += "<input placeholder='your google product category' title='Press `End.` on your keyboard in order to apply this value to all the sub-categories' type='text' class='mapping' onBlur='SGS_categories.updateSelection();' id='category_mapping_" + node['id'] + "' class='mapping' />";
        content += "</label>";
        content += "</div>";
        // enfants
        content += "</li>";
    ...
    selectChildren: function (parentId, cats) {
        var categories = {};
        if (typeof cats === "undefined" && $('simplegoogleshopping_categories').value !== '*' && $('simplegoogleshopping_categories').value !== '' && $('simplegoogleshopping_categories').value !== '[]') {
