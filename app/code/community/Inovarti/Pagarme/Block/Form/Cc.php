    ...
    public function getInstallmentsAvailables()
    {
        ...
        foreach ($collection as $item) {
            if ($item->getInstallment() == 1) {
                // $label = $this->__('Pay in full - %s', $quote->getStore()->formatPrice($total - $total * 0.05, false));
                $label = 'À vista (5% de desconto) - '.$quote->getStore()->formatPrice($quote->getSubtotal() - $quote->getSubtotal() * 0.05 + $quote->getShippingAddress()->getShippingAmount(), false);
            } else {
