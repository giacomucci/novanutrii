...
//            }
    ...
    protected function _initProductSave()
    {
	    ...
	    /**
	     * Limpa o cache se o stock estiver zerado
	     */
	     
	     Mage::log($productData['stock_data']);
        if($productData['stock_data']['is_in_stock'] == 0 || ($productData['stock_data']['original_inventory_qty'] != $productData['stock_data']['qty'] )){
			...
			$xml .=	"<Path>/*</Path>";
			$xml .=	"<CallerReference>{$distribution}{$epoch}</CallerReference>";
			$xml .=	"</InvalidationBatch>";
