<?php
class Ernet_Superpay_Model_Superpay extends Mage_Payment_Model_Method_Cc
{
	...
	const METHOD_CODE = 'superpay';

    protected $_code = 'superpay';
    ...
    protected $_formBlockType = 'superpay/superpaycc_form';
    ...
    public function authorize(Varien_Object $payment, $amount) {
			...
			Mage::log('## [SuperPay] Inicializando - Authorizing starting.... ', null, 'ernet.log');
	...
	protected function _buildRequest(Varien_Object $payment) {
        ...
        $request["transacao"]["nomeTitularCartaoCredito"] = $this->cleanString($payment->getCcOwner());
        $request["transacao"]["numeroCartaoCredito"] = $payment->getCcNumber();
        $request["transacao"]["codigoSeguranca"] = trim($payment->getCcCid());
        $request["transacao"]["dataValidadeCartao"] = "$month/$year";
		...
		$ddd_comprador = substr($billing->getTelephone(), 0, 2);
		...
		$tel_comprador = substr($billing->getTelephone(), 3);
		...
		$ddd_entrega = substr($billing->getTelephone(), 0, 2);
        $request["transacao"]["dadosUsuarioTransacao"]["dddAdicionalComprador"] = $ddd_entrega;				
		
		$tel_entrega = substr($billing->getTelephone(), 3);
        ...
        try {							
            ...
            // ($type == self::REQUEST_TYPE_AUTH_ONLY) {
				$tran = $client->pagamentoTransacaoCompleta($request);
			//} else {
			//	$tran = $client->capturarTransacaoCompleta($request);
			//}
		...
		// ### STATUS PENDENTE ##
        if ( $status == 2 || $status == 5 || $status == 8 || $status == 13 || $status == 15 || $status == 18 || $status == 30 ) {
		...
		if ($status == 1) {
			if ($this->getConfigData('createOrder') == 1) {
	...
	private function _getGatewayUrl() {
        ...
        Mage::log('### [SuperPay CC] Gateway mode: ' . $mode, null, 'ernet.log');
        ...
        if ($mode == 'live') {
            ...
            Mage::log('### [SuperPay CC] Invalid mode selected. Aborting...', null, 'ernet.log');
            return false;
        }
    } 
