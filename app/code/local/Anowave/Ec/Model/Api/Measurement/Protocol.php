...
class Anowave_Ec_Model_Api_Measurement_Protocol
{
	 ...
	 */
	public function purchase(Mage_Sales_Model_Order $order)
	{
		/**
		...
		try
		{
			...
			{
				Mage::getSingleton('core/session')->addNotice("Transaction tracking data (ID:{$order->getIncrementId()}) sent to Google Analytics successfully. (ID:$ua)");
		...
		{
			Mage::getSingleton('core/session')->addError('Transaction tracking failed due to error ' . $e->getMessage());
		}
